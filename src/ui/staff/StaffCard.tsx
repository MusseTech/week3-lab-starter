...
export default function StaffCard({ person }: Props) {
  // TODO: Use destructuring to extract values from the person
  ...
  const officeDisplay = officeLocation || "N/A";
  const coursesDisplay =
    courseIds && courseIds.length > 0 ? courseIds.join(", ") : "N/A";
  const specialtiesDisplay = specialties.slice(0, 3).join(", ");
  ...
  function logStaffInfo() {
    console.log(
      `Staff: ${name} | ${role} | ${school} | Office: ${officeDisplay}`,
    );
    if (startYear) {
      console.log(`Started: ${startYear}`);
    }
    console.log(`Teaches: ${coursesDisplay}`);
    console.log(`Specialties: ${specialtiesDisplay}`);
    if (funFact) {
  ...
  return (
    <Pressable style={styles.card} onPress={logStaffInfo}>
      ...
      <Text style={styles.meta}>Office: {officeDisplay}</Text>
      {startYear && <Text style={styles.meta}>Started: {startYear}</Text>}
      <Text style={styles.meta}>Teaches: {coursesDisplay}</Text>
      <Text style={styles.meta}>Specialties: {specialtiesDisplay}</Text>
